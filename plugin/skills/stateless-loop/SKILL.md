---
name: stateless-loop
description: Run an instruction in a stateless subprocess loop using `claude -p` until the task reports a CLEAN status or max iterations are reached. Use for repetitive fix-and-check workflows where each iteration starts with no LLM context from prior runs.
