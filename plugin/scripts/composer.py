...
def validate_manifest(
    ...
    # REQ-OVL-016: block unsupported schema versions by default.
    # REQ-OVL-074: forward compatibility via --allow-newer-schema flag.
...
def _validate_auxiliary_agent_file(
    ...
    # Parse tools list and reject forbidden tools (REQ-OVL-039).
...
def _collect_applied_content_files(
    ...
    contributions that composition will actually apply (REQ-OVL-051).
    """
    contribs = manifest.get("contributions", {})
...
def _build_contribution_index(
    ...
    targeting unknown anchors are silently excluded (REQ-OVL-051).
    """
...
def _generate_claude_md(
    ...
    # base content including safety blocks (REQ-OVL-045).
...
def compose(
    ...
    # 1a. Guard: project_path must not be inside or equal to base_path (REQ-OVL-042).
    ...
    if real_project == real_base or real_project.startswith(real_base + os.sep):
        return {
            ...
            "errors": [
                ...
                f"into the installed plugin (REQ-OVL-042)"
            ],
        }
    ...
    # Deterministic content fingerprint for idempotency (REQ-OVL-025).
    ...
    # (REQ-OVL-025 idempotency). Use fresh time only on first composition
    # or when inputs have changed (REQ-OVL-024, REQ-OVL-057).
    prev_lock_path = os.path.join(project_path, "spec", "overlay-manifest.lock")
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-injection",
        action="store_true",
        help="Proceed despite prompt injection warnings (REQ-OVL-064)",
    )
    parser.add_argument(
        "--allow-newer-schema",
        action="store_true",
        help="Allow overlays with unsupported schema_version (degraded mode, REQ-OVL-074)",
    )
    parser.add_argument(
        "--doctor",
        action="store_true",
