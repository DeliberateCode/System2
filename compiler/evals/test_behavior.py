...
class PathSafetyTest(unittest.TestCase):
    """project_path inside/equal to the base dir is refused."""
...
class ConflictRefusalParityTest(unittest.TestCase):
    """known-conflict refusal message byte-matches the oracle."""
...
class DryRunTest(unittest.TestCase):
    """dry_run computes files_to_write; the backend writes nothing."""
    ...
    def test_dry_run_computes_files_and_writes_nothing(self):
        ...
        try:
            ...
            self.assertEqual(
                [],
                os.listdir(proj),
                msg="dry-run emit must not write into the project dir",
            )
        finally:
            shutil.rmtree(proj, ignore_errors=True)
...
class AtomicWriteRestoreTest(unittest.TestCase):
    """a mid-write OSError restores backups with no partial output."""
