...
consolidation (TASK-005–011).

Consumed by `evals/test_old_world_lock.py` (TASK-011), which proves that feeding
this lock to `doctor` and `recompose_from_lock` yields **at most** the standard
stale-base nudge — never a schema error or a hard failure (REQ-046/071/043).

## What is "old-world" here

The compiler/composer engine bytes are identical pre/post consolidation (TASK-007's
...
- `content_fingerprint: sha256:8a53793d…` — the **genuine** fingerprint the engine
  ...
  content is byte-identical — exactly the REQ-043 semantic the test asserts.
