"""C3 / PG2 — committed per-target golden snapshots for Goose and Pi.

The existing Goose/Pi golden harnesses emit twice into two temp projects and
byte-diff the two trees against each other (a determinism check). That catches
NON-determinism but NOT a *render regression*: if the emitter changes shape but
still emits the same bytes on both runs (and ``goose recipe validate`` /
``discoverAndLoadExtensions`` still pass), the emit-twice gate stays green while the
artifacts silently drifted.
...
``evals/goldens_goose/<cell>/`` and ``evals/goldens_pi/<cell>/``; the regression
test re-emits a fresh tree and byte-diffs every file against the committed
snapshot. Any render drift is a hard failure.
...
(``system2.goose.lock.json`` / ``system2.pi.lock.json``) — an absolute fixture path
that varies by checkout location. It is normalized to the stable token
``<OVERLAY_SRC>`` on BOTH the freshly-emitted tree and the committed snapshot before
diffing. Nothing else is volatile (the emitters carry no timestamps). The relpath
listing itself is also asserted (no missing / extra files).
...
Stdlib-only ``unittest``; hermetic (temp projects; the real ``~/.config/goose`` /
``~/.pi`` are never touched by ``emit``).
"""
...
# The lock files that embed the volatile overlay source path.
_LOCKS = {
    "goose": "system2.goose.lock.json",
    "pi": "system2.pi.lock.json",
}
_SNAPSHOT_DIRS = {
    "goose": os.path.join(_THIS_DIR, "goldens_goose", _CELL),
    "pi": os.path.join(_THIS_DIR, "goldens_pi", _CELL),
}
_BACKENDS = {"goose": GooseBackend, "pi": PiBackend}
...
class _SnapshotRegression:
    ...
    target = None  # subclass sets "goose" / "pi"
...
class GooseSnapshotTest(_SnapshotRegression, unittest.TestCase):
    target = "goose"
