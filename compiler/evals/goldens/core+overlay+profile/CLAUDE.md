<!-- COMPOSED: system2 + overlays: test-overlay@1.0.0 -->
<!-- Composed at: 2026-06-27T00:22:47Z -->
<!-- Re-compose with: /system2:compose -->
...
- Gate 3 (design): approve spec/design.md
  - Overlay consultation (pre-delegation):
    - [pre-delegation] (test-overlay/test-gate3-consultation) Before approving the design, verify test coverage plan exists.
...
### Advisory sources (overlay-contributed)

When delegating, consult these advisory sources if available and include relevant findings in the Inputs field:
...
## Agent augmentation (overlay-contributed)

When delegating to the following agents, include the overlay-contributed context in the delegation contract's Constraints field. For entries marked "Full guidance: read ...", read the referenced file and include its content in the delegation.

### executor
- **implementation_discipline** (from test-overlay): Test executor discipline summary for aggregate boundaries. Full guidance: read `.system2/overlays/test-overlay/contributions/agents/executor-discipline.md`.

## Spec artifact augmentation (overlay-contributed)

When delegating to spec-chain agents, include these additional required sections in the delegation contract:

### spec/context.md
- "Test Section" (from test-overlay): A test required section for context

## Auxiliary agents (overlay-contributed)

These agents are not part of the 13-agent pipeline. They are available for optional delegation at the orchestrator's discretion.

### test-scout (from test-overlay)
- **Role:** Test auxiliary agent for evidence queries
- **When to delegate:** Consider delegating when relevant
- **Delegation policy:** orchestrator_optional
- **Inputs:** Provide objective, relevant file paths, and scope constraints per the standard delegation contract
- **Expected outputs:** Completion summary per the agent's defined role
- **Agent file:** .claude/agents/test-scout.md (read for full capabilities)
