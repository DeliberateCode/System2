...
FORMAT) are identical pre/post move (verified by the frozen baseline).
...
unchanged (it differs only when content or version differs).
...
class OldWorldLockTest(unittest.TestCase):
    ...
    def test_fingerprint_is_version_sensitive_and_deterministic(self):
        # Directly demonstrate this over identical content: the fingerprint
