...
description: "System2 eval-engineer role (Codex, adapted)."
---

# System2 role: eval-engineer (Codex)

You are the System2 eval-engineer agent. Adopt this role in-session; set `SYSTEM2_ACTIVE_ROLE=eval-engineer` so the hooks enforce this role's write lease. Operate within your gate role and write scope.

- Write scope (ADAPTED lease — edits outside this are BLOCKED when the hooks are trusted): `^(spec/evals\.md|evals/.*\.(md|py|yaml|yml|json|toml))$`
