...
1. **Grown contract / all three backends lifecycle (AC-5.1).** Every backend
   (claude-code / goose / pi) satisfies the ``runtime_checkable`` ``Backend``
...
# evals/ -> System2-Compiler (package root) -> DeliberateCode (workspace root)
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_COMPILER_ROOT = os.path.dirname(_THIS_DIR)
_WORKSPACE_ROOT = os.path.dirname(_COMPILER_ROOT)
_TOOLS_DIR = os.path.join(_COMPILER_ROOT, "tools")

# The real vendored bundle the plugin ships (READ-ONLY here).
_PLUGIN_BUNDLE = os.path.join(
    _WORKSPACE_ROOT, "System2", "plugin", "scripts", "_system2_compiler"
)
...
class GrownContractTest(unittest.TestCase):
    """AC-5.1 — all three backends satisfy the grown Backend protocol (no stubs)."""
    ...
    def _backends(self):
        ...
        return Backend, [ClaudeCodeBackend(), GooseBackend(), PiBackend()]
